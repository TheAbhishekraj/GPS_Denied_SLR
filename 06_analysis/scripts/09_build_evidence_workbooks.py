...
def recover_rec0115_span(text: str) -> str:
    """Recover visibly damaged spans only; preserve readable text and equations.

    U+0003 is the encoded space in this PDF. Offset +29 restores its ASCII
    glyphs (e.g. /RZ -> Low). Literal spaces inserted by extraction stay spaces.
    Spans without this marker are intentionally left for visual review.
    """
    if "\x03" not in text:
        return text
    return "".join(
        chr(ord(ch) + 29) if 3 <= ord(ch) <= 97 and ch != " "
        else ch
        for ch in text
    )
...
def build_pages(pdf: Path) -> list[str]:
    with fitz.open(pdf) as doc:
        if pdf.stem != "REC_0115":
            return [page.get_text() for page in doc]
        return [
            "\n".join(
                "".join(recover_rec0115_span(span["text"]) for span in line["spans"])
...
def main() -> int:
    ...
    ap.add_argument("--ids", nargs="+", help="build only these record IDs")
    args = ap.parse_args()
    ...
    if args.ids:
        missing = set(args.ids) - {pdf.stem for pdf in pdfs}
        if missing:
            ap.error("PDFs not found: " + ", ".join(sorted(missing)))
        pdfs = [pdf for pdf in pdfs if pdf.stem in args.ids]
    if args.limit:
